...
\framesubtitle{Why Wave Traversal is important for Distributed Systems?}
\begin{block}{Wave Traversal} 
Wave Traversal is important since in distributed systems it is important to traverse the network in a way that the nodes are visited with or without a specific order. For example, to collect information or to perform a specific task on the network all or some of the nodes should be visited. 
\end{block}
q\note{}
\end{frame}
